...
with open('data/house_Zipcode_Ll.csv', 'r') as csvfile:
    ...
    for row in csvReader:
        temp1 = []
        temp2 = []
        temp1.append(float(row[legend.index("long")]))
        train_Long = np.vstack((train_Long, temp1))
        temp2.append(float(row[legend.index("lat")]))
        train_Lat = np.vstack((train_Lat, temp2))
